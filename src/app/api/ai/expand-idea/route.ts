...
export async function POST(request: Request) {
  try {
    ...
    const prompt = `O Ivan teve a seguinte ideia de conteudo:

"${body.text.trim()}"

Expanda essa ideia em um plano de execucao pratico e detalhado. Retorne EXATAMENTE neste formato JSON:
  ...
  "steps": [
    {
      ...
      "tip": "dica pratica de execucao (angulo de camera, texto na tela, etc)"
    }
  ],
  "hook": "frase de abertura que prende a atencao",
  "cta": "call-to-action final sugerido",
  "bestTime": "melhor horario/dia para postar esse tipo de conteudo",
  "extraTips": ["dica extra 1", "dica extra 2"]
}

Regras:
- Pense como se fosse um diretor de conteudo dando o roteiro pro Ivan executar
...
- Mantenha o tom do Ivan: direto, sem enrolacao, dados reais quando possivel
